...
<form action="${basepath}/news/article" method="post" class="form-horizontal">
<input type="hidden" name="type" id="type" value="${e.type!""}">
	...
	<div class="row">
			<div class="col-md-6">
				<div class="btn-group">
					<#if checkPrivilege("/news/article/selectList")>
						<button class="btn btn-primary" onclick="selectList(this)" method="selectList">
						查询 <i class="fa fa-search"></i></button>
	                </#if>
					<#if checkPrivilege("/news/article/insert") >
						<a class="btn btn-success" href="${basepath}/news/article/toAdd">
						添加<i class="fa fa-plus"></i></a>
					</#if>
					<#if checkPrivilege("/news/article/deletes")>
					...
					<#list pager.list as item>
						<tr role="row">
							...
							<td>
			                    ...
			                    <#if checkPrivilege("/news/article/recommend")>
			                    	 <#if item.recommend == "0">
							...
							<td>
			                    ...
			                    <#if checkPrivilege("/news/article/stick")>
			                    	 <#if item.stick == "0">
							...
							<td>
								<#if checkPrivilege("/news/article/show")>
			                        <a href="${basepath}/news/article/show?id=${item.id}" target="_blank">预览</a>
			                    </#if>
			                    <#if checkPrivilege("/news/article/update")>
			                        <a href="${basepath}/news/article/toEdit?id=${item.id}">编辑</a>
			                    </#if>
			                    <#if checkPrivilege("/news/article/publish")>
			                    	 <#if item.state == "0">
...
function publish(obj, id,tip, flag){
	...
	if(confirm(tip)){
		$.ajax({
					url:"${basepath}/news/article/publish",
					type:"post",
					data:{ids:id},
					dataType:"text",
...
function recommend(obj, id,tip, flag){
	...
	if(confirm(tip)){
		$.ajax({
					url:"${basepath}/news/article/recommend",
					type:"post",
					data:{ids:id},
					dataType:"text",
					success:function(data){
...
function stick(obj, id,tip, flag, slt){
	if(confirm(tip)){
		$.ajax({
					url:"${basepath}/news/article/stick",
					type:"post",
					data:{ids:id},
					dataType:"text",
					success:function(data){

          ...
          <dl class="dl-horizontal clearfix">
            ...
            <a href="#">
            	<%if(StringHelper.isNotNull(comment.getImg())){ %>
            		<img alt="#" style="width:50px;height:50px;border-radius:100px;" src="${ctx }/upload/file/<%=comment.getImg() %>" />
            	<%}else{ %>
            		<img alt="#" style="width:50px;height:50px;border-radius:100px;" src="${ctx }/d/template/images/news_21.png" />
            	<%}%>
            </a>
            </dt>
            <dd>
              <div class="sky"><a><%=comment.getMname() %></a>
            ...
            <br /> <p class="talk"><a><%=comment.getConcent() %></a></p>
             ...
             <%if(StringHelper.isNotNull(request.getParameter("mid"))){ %> 
			        ...
			        <%}else if(comment.getIsaction().equals("c") ){%>
			        	...
			        	<div class="zan_a"><a onclick="addPraise('${param.mid}','${param.id}','cg','p')" style="cursor: pointer;color: red;" ><%=comment.getActionC() %></a></div>
                		<div class="buzan0" onclick="addPraise('${param.mid}','${param.id}','cg','p')" style="cursor: pointer;color: red;background:url(${ctx}/d/template/images/hongse-cai.png) no-repeat;"></div>
			         <%}else{%>
			        	<div class="zan_a">
			        	<a onclick="addPraise('${param.mid}','${param.id}','z','p')" style="cursor: pointer;" ><%=comment.getActionZ() %></a>
			        	</div>
			        	<div class="zan0" onclick="addPraise('${param.mid}','${comment.cid}','z','p')" style="cursor: pointer;background:url(${ctx}/d/template/images/huise-zan.png) no-repeat;"></div>
			        	<div class="zan_a">

...
<form action="${basepath}/statics/link" method="post" class="form-horizontal">
				...
				<tbody>
					<#list pager.list as item>
						<tr role="row">
							...
							<td>
								<a href="${item.link!""}" target="_blank" alt="${item.link!""}">
									<img style="height: 35px; width:140px;" src="${item.img}" alt="${item.name}" />
								</a>
							</td>
							<td>
			                    <#if item.status == "1">
							...
							<td>
			                    <#if checkPrivilege("/statics/link/update")>
			                        <a href="${basepath}/statics/link/toEdit?id=${item.id}">编辑</a>
			                    </#if>
			                    <#if checkPrivilege("/statics/link/work")>
			                     	<#if item.status == "0">
...
function work(obj, id,tip, flag){
	...
	if(confirm(tip)){
		$.ajax({
					url:"${basepath}/statics/link/work",
					type:"post",
					data:{ids:id},
					dataType:"text",
...
function statics(tip){
	if(confirm(tip)){
		$.ajax({
					url:"${basepath}/statics/link/statics",
					type:"post",
					data:{},
					dataType:"text",
					success:function(data){

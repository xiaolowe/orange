...
<form action="${basepath}/statics/catelog" method="post" class="form-horizontal">
	...
	<div class="row">
			<div class="col-md-6">
				<div class="btn-group">
					<#if checkPrivilege("/statics/catelog/selectList")>
						<button class="btn btn-primary" onclick="selectList(this)" method="selectList">
						查询 <i class="fa fa-search"></i></button>
	                </#if>
					<#if checkPrivilege("/statics/catelog/insert") >
						<a class="btn btn-success" href="${basepath}/statics/catelog/toAdd">
						添加<i class="fa fa-plus"></i></a>
					</#if>
					<#if checkPrivilege("/statics/catelog/deletes")>
						<a class="btn btn-danger" onclick="submitIDs(this,'确定删除选择的记录?');" method="deletes" >
						删除<i class="fa fa-times"></i></a>
	                 </#if>
	                <#if checkPrivilege("/statics/catelog/statics")>
			...
			<table class="table table-striped table-bordered table-hover dataTable no-footer" id="sample_1" role="grid" aria-describedby="sample_1_info">
				<thead>
					<tr role="row">
						...
						<th rowspan="1" colspan="1" aria-label="排序">
							排序
						</th>
				...
				<tbody>
					<#list pager.list as item>
						<tr role="row">
							...
							<td>
			                    <#if checkPrivilege("/statics/catelog/update")>
			                        <a href="${basepath}/statics/catelog/toEdit?id=${item.id}">编辑</a>
			                    </#if>
			                    <#if checkPrivilege("/statics/catelog/work")>
			                     	<#if item.status == "0">
...
function work(obj, id,tip, flag){
	...
	if(confirm(tip)){
		$.ajax({
					url:"${basepath}/statics/catelog/work",
					type:"post",
					data:{ids:id},
					dataType:"text",

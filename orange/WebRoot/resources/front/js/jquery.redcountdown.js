	...
	$.fn.redCountdown = function(Options, Callback) {
		...
		function Responsive() {
			...
			if (Settings.style.textResponsive) { 
				Element.find(".redCountdownValue").css("font-size", Math.floor(Element.find("> div").eq(0).width() * Settings.style.textResponsive / MAX_NUM_ZOOM) + "px");
				Element.find(".redCountdownValue").each( function() {
					$(this).css("margin-top",Math.floor(0 - (parseInt($(this).height()) / MAX_NUM_TOP)) + "px");
				});
			}
